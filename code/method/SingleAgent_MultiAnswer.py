...
class SingleAgent_MultiAnswer:
    ...
    def answer_(self, question, id, answer_list):
        ...
        answer = gen_response(self.config, self.tokenizer, self.model, messages)
    ...
    def answer(self, question):
        ...
        for _ in range(self.config.num):
            answer_list.append("")
            self.answer_(question, _, answer_list)
        
        messages=[
            {
                "role": "user",
                "content": f"Please go through the following answers and choose one answer that comes up the most: {answer_list}"
            }
        ]
